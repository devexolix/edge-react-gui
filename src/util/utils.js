...
export const displayFiatAmount = (fiatAmount?: number, precision?: number = 2, grouping?: boolean = false) => {
  ...
  return formatNumber(bns.toFixed(absoluteAmount, 2, precision), { grouping })
}
...
export function formatFiatString(props: { fiatAmount: string | number, minPrecision?: string | number, autoPrecision?: boolean, grouping?: boolean }) {
  const { fiatAmount, minPrecision = 2, autoPrecision = false, grouping = false } = props
  ...
  return displayFiatAmount(parseFloat(fiatAmtCleanedDelim), precision, grouping)
}
